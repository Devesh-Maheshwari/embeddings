# pretraining/word2vec/train_cbow.py
import argparse, json, yaml
from pathlib import Path
from typing import List, Union
from ..utils.evaluation.eval_intrinsic import evaluate_embeddings
from ..utils.benchmark_datasets import EmbeddingBenchmarkDatasets
from ..utils.tokenizer import simple_tokenizer
...
def main(cfg_path: str):
    ...
    # ===== hyperparams =====
    w2v_cfg = cfg["embeddings"]["word2vec_cbow"]
    model = CBOWSGNS(
        vector_size=int(w2v_cfg.get("vector_size", 300)),
        window=int(w2v_cfg.get("window", 10)),
        min_count=int(w2v_cfg.get("min_count", 50)),
        negative=int(w2v_cfg.get("negative", 15)),
        subsample_t=float(w2v_cfg.get("subsample_t", 1e-5)),
